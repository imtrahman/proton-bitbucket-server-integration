...
exports.handler = async (event) => {
    ...
    try {
        ...
        const repoConfig = {
            ...
            cHash: eventBody.changes[0].toHash
        };
        ...
        const s3Upload = await s3.upload({
          ...
          //  Key: `${repoConfig.projectName}/${repoConfig.repoName}/${repoConfig.branch}/${repoConfig.cHash}.tar.gz`,
            Key: `${repoConfig.repoName}/${repoConfig.cHash}.tar.gz`,
            Body: file
            Metadata: {
                'RepoName': `${repoConfig.repoName}`,
                'ProjectName': `${repoConfig.projectName}`,
                'CommitHash': `${repoConfig.cHash}`,
...
async function downloadFile(repoConfig, proxy) {
    ...
    const params = {
        ...
        url: `/rest/api/latest/projects/${repoConfig.projectName}/repos/${repoConfig.repoName}/archive?at=refs/heads/${repoConfig.branch}&format=tgz`,
        responseType: 'stream',
        headers: {
